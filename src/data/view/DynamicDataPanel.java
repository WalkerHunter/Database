...
public class DynamicDataPanel extends JPanel
{
	 ...
	 * 
	 * @param table
	 *            the Table that provides the reference for the input and
	 *            display components
	 */
	...
	{ // put layout code in here for offsets etc..
		// myBtn
	...
	private String getValueList()
	{
		...
		for (int index = 0; index < inputFieldList.size(); index++)
		{
			...
			{ // if last part, end with a ')'
				values += "'" + temp + "',)";
			}
			else
			{
				values += "'" + temp + "', "; // else if not end of list, add a
												// comma
			}
		}
